...
#if defined(NDEBUG)
# error "FullSendCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbdac64cd69c574230c20e16b2ce153fad77260a5b53c4af6c75b7107b1b83fca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FullSendCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FullSendCoin Signed Message:\n";
...
{
    // FullSendCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // FullSendCoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // FullSendCoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // FullSendCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FullSendCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FullSendCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2f5bce7a4779d2088fb4be7dd69feee22040fd942d1854084fc2c17dfc3abbd7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3/18/22 - The First Ever Block is Mined! Only took 2 days to setup!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cb817f4256c0fbb778222c2b93320040db07bfd33674c8edf9c40a9f723307daceb0397a7d45a805a322eba98696bf4b2663b537e8cc970e4bc730e8bf64c39e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1647635467;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085670216;

        if (fTestNet)
        {
            block.nTime    = 1647635419;
            block.nNonce   = 385286734;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x294df4783c7b78a9b9efa80ef81cb6568edde3f9236213d43a5cebddeeefbde6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc9, 0xb2, 0xdf }; // FullSendCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FullSendCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FullSendCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FullSendCoinMiner(CWallet *pwallet)
{
    printf("FullSendCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FullSendCoin-miner");
    ...
    try { loop {
        ...
        printf("Running FullSendCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FullSendCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FullSendCoinMiner, pwallet));
}
